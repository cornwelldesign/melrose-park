...
export default class MultiGroup extends React.Component {
  ...
  selectPage(nPage){    //Set the current page on the state
      this.setState({nPage});
			...
			if((this.state.nPage + 1) == this.state.count ) {
  ...
  render(){
    ...
    return (
        <div onWheel={this.onWheel} onTouchStart={this.onTouchStart} onTouchMove={this.onTouchMove} onTouchEnd={this.onTouchEnd} className={`${this.state.scroller} reached-${this.state.reached}`}>
            ...
            <div className="right" style={{bottom:`-${this.state.height*(nPage-1)}px`,transition}}>
                {this.renderList(contRight)}
						...
						<Menu onPress={this.selectPage}
                    step={this.state.nPage + 1} className="down-arrow" ><img src="/svg/melrose-park-sydney-property-004-DOWN.svg" /></Menu>
...
class Menu extends React.Component {
   constructor(props){
    ...
    render(){
      ...
      return <li className={`${select}`}><a href="#" className={this.props.className} onClick={this.onClick}>{this.props.children} </a></li>;
      }
}
